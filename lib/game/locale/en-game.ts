// Тоглоомын зурвас, сануулга, зорилго, эд зүйл, улирал, дайсны нэрс.
// Түлхүүр = кодод байгаа монгол мөр яг тэр хэлбэрээр.

export const EN_GAME: Record<string, string> = {
  " · Эхлээд ноос г.м зарж зоос цуглуул":
    " · Sell wool and the like first to earn coins",
  "+ ХӨХ ТЭНГЭРИЙН СЭЛЭМ": "+ SKY-BLUE SWORD",
  "+1 жимс": "+1 berries",
  "+1 загас": "+1 fish",
  "+1 морь!": "+1 horse!",
  "+1 ноолуур": "+1 cashmere",
  "+1 ноос": "+1 wool",
  "+1 сүү": "+1 milk",
  "+1 тэмээний ноос": "+1 camel wool",
  "+1 чулуу": "+1 stone",
  "+28 хоол": "+28 food",
  "+36 хоол": "+36 food",
  "+36 хоол · +20 амь": "+36 food · +20 HP",
  "+ааруул": "+aaruul",
  "1 ааруул → 30 зоос": "1 aaruul → 30 coins",
  "1 загас → 12 зоос": "1 fish → 12 coins",
  "1 ноолуур → 22 зоос": "1 cashmere → 22 coins",
  "1 ноос → 8 зоос": "1 wool → 8 coins",
  "1 сүү → 6 зоос": "1 milk → 6 coins",
  "1 эсгий → 45 зоос": "1 felt → 45 coins",
  "1000 толгойд хүргэ.": "the herd to 1000 head.",
  "CHEAT: Бодит ертөнцөд буцлаа.": "CHEAT: Returned to the real world.",
  "CHEAT: Галын үеийг алгаслаа.": "CHEAT: Skipped the fire chapter.",
  "CHEAT: Гэр бүлийн уулзалтыг алгаслаа.":
    "CHEAT: Skipped the family reunion.",
  "CHEAT: Мал, нар жаргах үеийг алгаслаа.":
    "CHEAT: Skipped the livestock and sunset chapter.",
  "CHEAT: Нээлтийн хэсгийг эхлүүллээ.": "CHEAT: Started the opening chapter.",
  "CHEAT: Оршил алгаслаа.": "CHEAT: Skipped the prologue.",
  "CHEAT: Сүнсний ертөнц — дөнгөж орсон. Туслахуудыг дар.":
    "CHEAT: Spirit world — just arrived. Beat the helpers.",
  "CHEAT: Сүнсний замыг нээлээ.": "CHEAT: Opened the spirit path.",
  "CHEAT: Таван сахиулыг дарлаа.": "CHEAT: Beat the five guardians.",
  "CHEAT: Тулааны сургалтыг алгаслаа.": "CHEAT: Skipped combat training.",
  "CHEAT: Төмөр шулмасыг дарлаа.": "CHEAT: Beat Tömör Shulmas.",
  "CHEAT: Хар төмөр хаалгыг нээлээ.": "CHEAT: Opened the black iron gate.",
  "CHEAT: Хөх тэнгэрийн сэлмийг авлаа.": "CHEAT: Took the Sky-Blue Sword.",
  "CHEAT: Шулмасыг дийлээд аав ээжтэй амьдрах үе эхэллээ.":
    "CHEAT: Demon beaten — life with mother and father begins.",
  "CHEAT: Шулмасын баатрыг дарлаа.": "CHEAT: Beat the Shulmas Warrior.",
  "CHEAT: Шулмасын баатрын тулааныг эхлүүллээ.":
    "CHEAT: Started the Shulmas Warrior fight.",
  "CHEAT: Шуурганы мөрийг шинжиллээ.": "CHEAT: Examined the storm's trail.",
  "CHEAT: Үүрийн өвгөний яриаг алгаслаа.":
    "CHEAT: Skipped the dawn talk with the elder.",
  "CHEAT: Өвгөний заавар хүртэл алгаслаа.":
    "CHEAT: Skipped ahead to the elder's lesson.",
  "CHEAT: Өвгөний танилцах яриаг алгаслаа.":
    "CHEAT: Skipped meeting the elder.",
  "Cheat унтарлаа.": "Cheats off.",
  "E / Enter / Space — алгасах": "E / Enter / Space — skip",
  "E / Enter / Space — оршилыг бүтнээр алгасах":
    "E / Enter / Space — skip entire intro",
  "E / F — Гал асаах (3 түлээ)": "E / F — Light fire (3 firewood)",
  "E / F — Зууханд түлээ хийж гал асаах (3)":
    "E / F — Add firewood and light the stove (3)",
  "E дарж гэрийн зүг буц.": "Press E to head home.",
  "E — Mini-boss-ийн талбайг шалгах": "E — Scout the mini-boss ground",
  "E — Авдар": "E — Chest",
  "E — Гэрт орож зууханд гал асаа": "E — Enter the ger, light the stove",
  "E — Гэрт орох · G — моринд ачих": "E — Enter ger · G — load on horse",
  "E — Гэрт орох / бууж уях · G — моринд ачих":
    "E — Enter ger / dismount, hitch · G — load on horse",
  "E — Загас татах!": "E — Reel it in!",
  "E — Загас залгах!": "E — Hook the fish!",
  "E — Дэгээ залгах!": "E — Set the hook!",
  "E — ХУРДАН ТАТ! (mash)": "E — MASH to reel!",
  "E — Уургалах (хазтал хүлээ)": "E — Cast (wait for a bite)",
  "E — Мал гаргах · J — Нураах": "E — Let livestock out · J — Tear down",
  "E — Мал оруулах · J — Нураах": "E — Bring livestock in · J — Tear down",
  "E — Мод хагалах": "E — Chop wood",
  "E — Мод хагалах (сүх)": "E — Chop wood (axe)",
  "E — Ордноос буцах": "E — Leave the palace",
  "E — Түгжээтэй · эхлээд Шулмасын баатрыг ял":
    "E — Locked · beat the Shulmas Warrior first",
  "E — Түлээ нэмэх": "E — Add firewood",
  "E — Зууханд түлээ нэмэх": "E — Add firewood to the stove",
  "E — Төмөр шулмасын ордонд орох": "E — Enter Tömör Shulmas's palace",
  "E — Унтах": "E — Sleep",
  "E — Урлал": "E — Craft",
  "E — Уургалах (загас ойртохыг хүлээ)": "E — Cast (wait for a fish)",
  "E — Хар мөрийн тухай өгүүл": "E — Tell of the dark trail",
  "E — Хөх тэнгэрийн сэлмийг авах": "E — Take the Sky-Blue Sword",
  "E — Шуурганы мөрийг шинж": "E — Examine the storm's trail",
  "E — бодит ертөнц рүү буцах": "E — return to the real world",
  "E — Өвгөнтэй уулз": "E — Meet the elder",
  "E — Өвгөнтэй ярилц": "E — Talk to the elder",
  "E — Өвгөнтэй ярих / арилжаа": "E — Talk to elder / trade",
  "E — Өвс хадаад тэвшид хий": "E — Cut hay into the trough",
  "E — Өвс хадах": "E — Cut hay",
  "G — Гэр буулгах (мориноос)": "G — Unload ger (from horse)",
  "E — морь унах": "E — mount horse",
  "E — морьноос буух": "E — dismount horse",
  "H — морь унах": "H — mount horse",
  "H — морьноос буух": "H — dismount horse",
  "J — ТӨГСГӨЛ": "J — FINISH",
  "J / Attack": "J / Attack",
  "J — Хаалга нураах": "J — Tear down gate",
  "J — Хашаа нураах": "J — Tear down fence",
  "L дарж, дайралтыг нь няцаа.": "press L to turn its blow aside.",
  "L / Parry дарж няцаа.": "press L / Parry to turn it aside.",
  "Mini-boss · Шулмасын баатар": "Mini-boss · Shulmas Warrior",
  "Mini-boss унав · Сэлмээ ав": "Mini-boss down · Take the sword",
  "N — Малаа туу": "N — Herd livestock",
  "Posture эвдэрлээ — одоо J: амийн 1/4!":
    "Posture broken — now J: a quarter of its life!",
  "Story дууссан. Одоо сүргээ 1000 толгойд хүргэ.":
    "The story is done. Now grow your herd to 1000 head.",
  "Сүргээ 1000 толгойд хүргэлээ!": "You grew the herd to 1000 head!",
  "WASD — алхах · E — харьцах · Зураг дээр дарж томруул · Хаалга руу алхаж гарна":
    "WASD — walk · E — interact · Click a painting to zoom · Walk to the door to leave",
  А: "A",
  АСУУЛТ: "QUESTION",
  "АСУУЛТ · ЯРИА": "QUESTION · TALK",
  Аав: "Father",
  "Аав +1 жимс": "Father +1 berries",
  "Аав +1 чулуу": "Father +1 stone",
  "Аав гэртээ": "Father is home",
  "Аав цохив": "Father strikes",
  "Аав, ээжийг хүлсэн": "Defeat the iron demon that",
  "Аав, ээжийг хүлсэн төмөр шулмасыг ял.":
    "Defeat the iron demon that bound your mother and father.",
  "Ааруул зарах": "Sell aaruul",
  Авдар: "Chest",
  "Айдас арилаагүй ч хүү түүнд захирагдсангүй.":
    "The fear did not leave, yet the boy did not bow to it.",
  "Алсад чонын улиан сонсогдоно.": "Far off, a wolf's howl carries.",
  Анхан: "Basic",
  "Аюул ойрхон — эхлээд тулалдаа!": "Danger nearby — fight first!",
  "Ахих боломжтой": "Can level up",
  Б: "B",
  "БАРИУЛАВ!": "CAUGHT!",
  "БИЕ СУЛАРЛАА!": "STAGGERED!",
  "БОДИТ ЕРТӨНЦ": "REAL WORLD",
  Баавгай: "Bear",
  "Баавгай бариад хазчихлаа — амьд үлдлээ!":
    "The bear caught and bit you — you lived!",
  "Баавгай мал руу дайрлаа — маш аюултай!":
    "A bear is on the livestock — very dangerous!",
  "БААВГАЙ ИРЛЭЭ!": "BEAR APPROACHING!",
  "Бэлэн бай — хамгаал!": "Get ready — defend!",
  "Баавгайд ялагдлаа…": "The bear beat you…",
  "Баавгайн сүнс одлоо.": "The bear's spirit departs.",
  "Баавгайн сүнс одлоо — ойрхон аюул үлдлээ.":
    "The bear's spirit departs — danger nearby remains.",
  Баярцогт: "Bayartsogt",
  Билгүүнтөгс: "Bilgüüntögs",
  "Бодит ертөнцөд буц": "Return to the real world",
  Буулаа: "Dismounted",
  "Бэлтгэл хийгээд дахин ир.": "Prepare, then come again.",
  "Бэлчээр хоосон! G-ээр нүүж шинэ бэлчээр ол, эсвэл улирал хүлээ.":
    "Pasture is bare! Move with G to find new grass, or wait for the season.",
  "Бэлчээрийн өвс бага.": "The pasture grass is running low.",
  "Бэлчээрийн өвс дууслаа! Гэрээ хурааж (G) шинэ бэлчээр рүү нүү, эсвэл тэвш ашигла.":
    "The pasture grass is gone! Pack the ger (G) and move on, or use the trough.",
  "Бэлчээрт үлдсэн малыг чоно идэв! Хашаанд оруул!":
    "Wolves took the livestock left on pasture! Get them in the pen!",
  "Бэлчээрт үлдсэн малыг чоно идэв! Орой бүр хашаанд оруул!":
    "Wolves took livestock left on pasture! Bring them in every evening!",
  "Бүх мал үгүй болов… Ялагдлаа.": "Every head is gone… You have lost.",
  В: "C",
  Г: "D",
  "ГАЛ АСЛАА": "THE FIRE IS LIT",
  "ГЭР БҮЛ ЭРГЭН НЭГДЭВ": "THE FAMILY IS WHOLE AGAIN",
  "Гал асааж байна…": "Lighting the fire…",
  "Гал асаалаа.": "The fire is lit.",
  "Галаа асаа": "Light your fire",
  "Галын дэргэд хашаа барихгүй.": "No fence next to the fire.",
  "Гарах — P": "Exit — P",
  "Голомт сэргэв! Зуухны гал ассан.":
    "The hearth lives again! The stove is burning.",
  "Голомт унтарсан байна. Түлээ бэлтгэж, гэртээ орж зууханд гал асаа.":
    "The hearth has gone cold. Gather firewood, go inside and light the stove.",
  "Голомтоо сахиж, сүргээ": "Keep your hearth and grow",
  "Голомтоо сахиж, сүргээ 1000 толгойд хүргэ.":
    "Keep your hearth and grow the herd to 1000 head.",
  "Голомтын дэргэд суух": "Go to the strange old man",
  "Голомтын дэргэд суух үл таних өвгөн дээр оч.":
    "Go to the strange old man sitting by the hearth.",
  "Голомтын дэргэд үл таних өвгөн тайван сууж байлаа.":
    "By the hearth sat a strange old man, calm as still water.",
  "Голоос загас барина · Q-аар иднэ": "Catch fish in the river · Q to eat",
  "Голын эрэг дээр зогсож уургална.": "Cast from the river bank.",
  "Гэр буулаа!": "The ger is pitched!",
  "Гэр бүл": "Family",
  "Гэр моринд ачсан — эхлээд G-ээр буулга.":
    "The ger is on the horse — unload it with G first.",
  "Гэр → морь": "Ger → horse",
  "Гэрийг моринд ачлаа. Шинэ бэлчээр олоод G дарж буулга.":
    "The ger is loaded. Find new pasture and press G to unload.",
  "Гэрийн дэргэд хашаа барихгүй.": "No fence next to the ger.",
  "Гэрийнхээ дэргэд зогсоод G дар — моринд ачна.":
    "Stand by your ger and press G — it loads onto the horse.",
  "Гэртээ орж зууханд гал асаа": "Go inside and light the stove",
  "Гүйх хурд +12%": "Run speed +12%",
  "Дайралтыг сөр": "Parry the attack",
  "Дараад / Esc — хаах": "Click / Esc — close",
  "Дараад хаах · Esc": "Click to close · Esc",
  "Дулаан дээл": "Warm deel",
  Дунд: "Mid",
  "Дутуу XP": "XP short",
  Дуусгах: "Finish",
  Дээд: "High",
  "Дээд амь +25, бүрэн эдгэрнэ": "Max health +25, full heal",
  "Загас барьлаа! Q дарж идээрэй.": "A fish! Press Q to eat it.",
  "Цэнхэр загас (амархан) барьлаа! Q дарж идээрэй.":
    "Blue fish (easy) caught! Press Q to eat.",
  "Ногоон загас (хэцүү) барьлаа! Q дарж идээрэй.":
    "Green fish (hard) caught! Press Q to eat.",
  "Алтан загас (маш хэцүү) барьлаа! Q дарж идээрэй.":
    "Gold fish (very hard) caught! Press Q to eat.",
  "Загас зарах": "Sell fish",
  "Загас ойртоогүй — дэнсээ хүлээ.": "No fish yet — wait for a bite.",
  "Загас дэгээг хазтал хүлээ — дараа нь E дар.":
    "Wait for a bite — then press E.",
  "Загас дэгээг хазлаа! E дарж залга!": "Bite! Press E to set the hook!",
  "Цэнхэр загас (амархан) хазлаа! E дарж залга!":
    "Blue fish (easy) bite! Press E!",
  "Ногоон загас (хэцүү) хазлаа! E дарж залга!":
    "Green fish (hard) bite! Press E!",
  "Алтан загас (маш хэцүү) хазлаа! E дарж залга!":
    "Gold fish (very hard) bite! Press E!",
  "Том загас хазлаа! E дарж залга!": "Big fish bite! Press E!",
  "ХОВОР том загас хазлаа! E дарж залга!": "RARE huge bite! Press E!",
  "Залгалаа! E-г хурдан дарж тат!": "Hooked! Mash E to reel it in!",
  "Цэнхэр загас (амархан) залгалаа! E-г хурдан дарж тат!":
    "Blue fish (easy) hooked! Mash E!",
  "Ногоон загас (хэцүү) залгалаа! E-г хурдан дарж тат!":
    "Green fish (hard) hooked! Mash E!",
  "Алтан загас (маш хэцүү) залгалаа! E-г МАШ хурдан дар!":
    "Gold fish (very hard) hooked! MASH E!",
  "Том загас залгалаа! E-г хурдан дарж тат!":
    "Big fish hooked! Mash E hard!",
  "Ховор загас залгалаа! E-г МАШ хурдан дар!":
    "Rare fish hooked! MASH E!",
  "E хурдан дар — загас зугтаж байна!": "Mash E — the fish is escaping!",
  "E хурдан дар — цэнхэр загас (амархан)!":
    "Mash E — blue fish (easy)!",
  "E хурдан дар — ногоон загас (хэцүү)!": "Mash E — green fish (hard)!",
  "E МАШ хурдан дар — алтан загас (маш хэцүү)!":
    "MASH E — gold fish (very hard)!",
  "E хурдан дар — том загас хүчтэй!": "Mash E — the big one fights hard!",
  "E МАШ хурдан дар — ховор загас зугтана!":
    "MASH E — the rare fish is breaking free!",
  "Том загас барьлаа! Q дарж идээрэй.": "Caught a big fish! Press Q to eat.",
  "Ховор том загас барьлаа! Q дарж идээрэй.":
    "Caught a rare fish! Press Q to eat.",
  "E — Цэнхэр загас тат! (амархан)": "E — Reel blue fish! (easy)",
  "E — Ногоон загас тат! (хэцүү)": "E — Reel green fish! (hard)",
  "E — Алтан загас тат! (маш хэцүү)": "E — Reel gold fish! (very hard)",
  "E — Цэнхэр загас тат!": "E — Reel blue fish!",
  "E — Ногоон загас тат!": "E — Reel green fish!",
  "E — Алтан загас тат!": "E — Reel gold fish!",
  "Цэнхэр загас барьлаа! Q дарж идээрэй.": "Blue fish caught! Press Q to eat.",
  "Ногоон загас барьлаа! Q дарж идээрэй.": "Green fish caught! Press Q to eat.",
  "Алтан загас барьлаа! Q дарж идээрэй.": "Gold fish caught! Press Q to eat.",
  "Цэнхэр загас хазлаа! E дарж залга!": "Blue fish bite! Press E!",
  "Ногоон загас хазлаа! E дарж залга!": "Green fish bite! Press E!",
  "Алтан загас хазлаа! E дарж залга!": "Gold fish bite! Press E!",
  "Цэнхэр загас залгалаа! E-г хурдан дарж тат!": "Blue fish hooked! Mash E!",
  "Ногоон загас залгалаа! E-г хурдан дарж тат!": "Green fish hooked! Mash E!",
  "Алтан загас залгалаа! E-г хурдан дарж тат!": "Gold fish hooked! Mash E!",
  "Алтан загас залгалаа! E-г МАШ хурдан дарж тат!":
    "Gold fish hooked! MASH E!",
  "E хурдан дар — Цэнхэр загас!": "Mash E — blue fish!",
  "E хурдан дар — Ногоон загас!": "Mash E — green fish!",
  "E хурдан дар — Алтан загас!": "Mash E — gold fish!",
  "E МАШ хурдан дар — Цэнхэр загас!": "MASH E — blue fish!",
  "E МАШ хурдан дар — Ногоон загас!": "MASH E — green fish!",
  "E МАШ хурдан дар — Алтан загас!": "MASH E — gold fish!",
  Цэнхэр: "Blue",
  "Цэнхэр загас": "Blue fish",
  "Ногоон загас": "Green fish",
  "Алтан загас": "Gold fish",
  амархан: "easy",
  хэцүү: "hard",
  "маш хэцүү": "very hard",
  "+2 загас": "+2 fish",
  "+3 загас": "+3 fish",
  "Загас зугтлаа!": "The fish got away!",
  "Загас зугтлаа! E-г хурдан дар.": "It escaped! Mash E faster next time.",
  "Загас шугам таслаад зугтлаа!": "The line snapped — fish escaped!",
  "Загасны уурга": "Fishing pole",
  "Загасны уурга хэрэгтэй — өвгөнөөс ав.":
    "You need a fishing pole — get one from the elder.",
  "Залуу мал хүйтэнд үхлээ! Гал түлж хашаанд байлга.":
    "Young stock died of cold! Keep a fire and pen them.",
  "Зугтлаа!": "Bolted!",
  зуух: "stove",
  "Зууханд гал асаалаа!": "The stove is lit!",
  "Зууханд гал асаалаа! Голомт сэргэж байна…":
    "The stove is lit! The hearth is waking…",
  "Зууханд түлээ нэмлээ.": "Firewood added to the stove.",
  "Зуухны гал унтарлаа.": "The stove has gone out.",
  "Зэрлэг морийг уургална": "Lassoes a wild horse",
  "Зэрлэг морь уургаар барив!": "Lassoed a wild horse!",
  "Зүүн хойших чулуун завсарт очиж,": "Go to the rocks to the northeast",
  "Их амь": "Great vigor",
  "МАЛ БҮРДЭВ": "THE HERD IS WHOLE",
  "Мал бэлчээрт!": "Livestock on pasture!",
  "Мал бүрдэв!": "The herd is whole!",
  "Мал оллоо": "Found livestock",
  "Мал хаалгаар бэлчээрт гарч байна.":
    "The livestock are filing out to pasture.",
  "Мал хаалгаар хашаандаа орж байна.":
    "The livestock are filing into the pen.",
  "Мал хашаа руу…": "To the pen…",
  "Мал хашаандаа орж амжлаа.": "The livestock made it into the pen.",
  "Мал өлсөж үхэж байна! Тэвшид өвс хий!":
    "The livestock are starving! Put hay in the trough!",
  "Малаа эрж ол": "Find your livestock",
  "Нүүдэлчин survival тоглоом": "Nomad — Mongolian herder survival",
  "Нүүдэлчин — Survival": "Nomad — Survival",
  "Мод/түлээ нэг цохилтоор унагана": "Fells trees and firewood in one hit",
  "Бутны бүх жимсийг нэг даралтаар түүнэ":
    "Picks every berry on a bush in one press",
  Сагс: "Basket",
  "Модны дээр хашаа барихгүй.": "No fence on top of a tree.",
  "Модон хашаа": "Wooden fence",
  "Монгол малчны амьдрал: сүрэг хамгаалах, чонотой тулалдах, 1000 хонь цуглуулах":
    "A Mongolian herder's life: guard the herd, fight wolves, gather 1000 sheep",
  "Морины зураг": "Horse painting",
  "Морь (сүрэг)": "Horse (herd)",
  "Морь гарлаа — уургаа хүзүү рүү шид (E)!":
    "A horse! Throw the lasso at its neck (E)!",
  "Морь гарлаа — уургатай ойртож E!":
    "A horse! Close in with the lasso and press E!",
  "Морь зугтав — ойртож дахин оролд!":
    "The horse bolted — get close and try again!",
  "Морь зугтав — E-г хурдан дар!":
    "The horse bolted — mash E faster!",
  "Уурга хүзүүнд ороогүй — дахин шид!":
    "The lasso missed the neck — throw again!",
  "Уурга нисэж байна…": "Lasso in the air…",
  "Уурга шидлээ — хүзүү рүү!": "Lasso thrown — toward the neck!",
  "Уурга тасарлаа — ойртож дахин шид!":
    "The lasso snapped — get closer and throw again!",
  "Уурга алдлаа!": "You lost the lasso!",
  "Хүзүүнд орлоо! E-г хурдан дарж тат!":
    "Around the neck! Mash E to pull it in!",
  "E хурдан дар — морь зугтаж байна!":
    "Mash E — the horse is thrashing!",
  "E — Уургаа хүзүү рүү шид": "E — Throw lasso at the neck",
  "Морь ойрхон байх ёстой — гадаа уясан морь руу оч.":
    "A horse must be near — go to the one hitched outside.",
  "Морь уналаа. E — буух.": "Mounted up. E — dismount.",
  "Морь уналаа. H — буух.": "Mounted up. H — dismount.",
  "Морь уялаа": "Horse hitched",
  "Морь хамгаалав": "The horse took it",
  "Морь чинь үхлээ… Дэлгүүрээс шинийг ав.":
    "Your horse is dead… Buy a new one at the shop.",
  "Морь үхсэн — дэлгүүрээс шинээр ав.":
    "The horse is dead — buy a new one at the shop.",
  "Морь үхэв!": "The horse is dead!",
  "Морь үхэж гэр унав! Энд буулаа — шинийг авч нүү.":
    "The horse died and the ger came down! It stands here — buy another and move.",
  "Морьноос бууж уялаа. E — дахин унах.":
    "Dismounted and hitched. E — mount again.",
  "Морьноос бууж уялаа. H — дахин унах.":
    "Dismounted and hitched. H — mount again.",
  "Морьноос буулаа.": "Off the horse.",
  Мянганнаст: "Myanganast",
  "НАЯН ЕСӨН ШИДТЭЙ ТӨМӨР ШУЛМАС": "TÖMÖR SHULMAS OF EIGHTY-NINE SPELLS",
  "Нар жаргаж байна! Малаа хашаанд оруул — чоно ирнэ!":
    "The sun is setting! Pen your livestock — the wolves come!",
  "Нар жаргаж байна! Малаа заавал хашаанд оруул — чоно ирнэ!":
    "Sunset! You must pen the livestock — wolves are coming!",
  "Нар мандахад зүүн толгодын": "At sunrise, go to the elder's",
  "Нар мандахад зүүн толгодын өвгөний бууцанд оч.":
    "At sunrise, go to the elder's camp in the eastern hills.",
  "Нар уулын цаагуур шингэж, тал нутгийг бараан сүүдэр нөмрөв.":
    "The sun sank beyond the mountains, and a dark shadow covered the steppe.",
  "Наян есөн шидтэй төмөр шулмаст ялагдлаа…":
    "The iron demon of eighty-nine spells beat you…",
  "Наян есөн шидтэй шулмас нам дор сөхрөв.…":
    "The demon of eighty-nine spells sank low to its knees.…",
  Номин: "Nomin",
  "Ноолуур зарах": "Sell cashmere",
  "Ноос зарах": "Sell wool",
  Нохой: "Dog",
  "Нохой чинь үхлээ… Дэлгүүрээс шинийг ав.":
    "Your dog is dead… Buy a new one at the shop.",
  "Нохой үхэв!": "The dog is dead!",
  "Нударгаа зангидлаа.": "You clench your fists.",
  Нум: "Bow",
  "Нум авлаа! +6 сум. Урлалаар дахин хийж болно.":
    "Got a bow! +6 arrows. Craft more at the workbench.",
  Нураав: "Torn down",
  "Нээлттэй мөчид цохь": "Strike the opening",
  "Нүүдэлд унах морь хэрэгтэй! Авдраас морь авч, гэрээ моринд ачна.":
    "You need a riding horse to move! Buy one from the chest, then load the ger.",
  "ОДООГИЙН ЗОРИЛГО": "CURRENT OBJECTIVE",
  "Одоогийн XP": "Current XP",
  "Ойрхон мод/жимс/чулуу/бэлчээр алга.":
    "No tree, berries, stone or pasture nearby.",
  "Орой боллоо! Мал хашаандаа. Шөнө ойртож байна":
    "Evening! Livestock penned. Night is closing in",
  "Орой боллоо! Малаа тууж хашаанд оруул. E — оруулах":
    "Evening! Drive the livestock into the pen. E — bring in",
  "Орой боллоо! Малаа заавал тууж хашаанд оруул. E — оруулах":
    "Evening! You must drive the livestock into the pen. E — bring in",
  "Орой/шөнө мал бэлчээрт гаргах аюултай! Хашаандаа байлга.":
    "Don't let them out at evening/night — keep them penned!",
  "Орой — мал хашаандаа. Амрах цаг.":
    "Evening — livestock penned. Time to rest.",
  "Сахиулын мөр тасарлаа.": "The guardians' line is broken.",
  "Сахиулын мөр тасарлаа. Хараалт хаалгыг зорь.":
    "The guardians' line is broken. Make for the cursed gate.",
  "Сая унтсан…": "You just slept…",
  "Саяхан унтсан шүү дээ.": "You slept only a moment ago.",
  "Стамина +40% хурдан нөхөгдөнө": "Stamina refills 40% faster",
  "Суллав!": "Let go!",
  "Сум алга — урлалаар хий (1 мод + 1 чулуу = 2 сум).":
    "No arrows — craft them (1 wood + 1 stone = 2 arrows).",
  "СҮНСНИЙ ОРОН": "THE SPIRIT REALM",
  "Сүнс дууссан… бодит ертөнц рүү буцлаа.":
    "No spirits left… back to the real world.",
  "Сүнс зарцуулав — дахин амиллаа!": "A spirit spent — you live again!",
  "Сүнсний дайснууд · E/P — гарах": "Spirit foes · E/P — leave",
  "Сүнсний дайснууд унав! E — бодит ертөнц рүү буцах.":
    "The spirit foes have fallen! E — return to the real world.",
  "Сүнсний замыг нээ": "Open the spirit path",
  "Сүнсний орноос буцлаа. Аав ээжийн мөр… үргэлжлүүлнэ.":
    "Back from the spirit realm. Your parents' trail… goes on.",
  "Сүнсний орноос гарлаа.": "You left the spirit realm.",
  "Сүнсний оронд Хар төмөр хаалга нээгдэж, boss тулаан эхэллээ.":
    "In the spirit realm the black iron gate opened — the boss fight begins.",
  "Сүнсний орон… цаг зогсов. Шулмасын туслахуудыг цэвэрлэ. E — буцах.":
    "The spirit realm… time halts. Clear the demon's helpers. E — back.",
  "Сүргийг чононоос хамгаална": "Guards the herd from wolves",
  "Сүргийн морь · сүү өгнө": "Herd horse · gives milk",
  "Сүргийн тэмээ · сүү/ноос": "Herd camel · milk/wool",
  "Сүргийн үхэр · сүү өгнө": "Herd cow · gives milk",
  "Сүргээ хамгаал": "Protect your herd",
  "Сүргээ өсгө": "Grow your herd",
  "Сүргээсээ холдуулж, амьд үлд.": "Draw it from the herd and survive.",
  Сүх: "Axe",
  "Сүү зарах": "Sell milk",
  "Тайвшир, хүү минь.": "Be still, my boy.",
  "Талын харагч": "Steppe Watcher",
  "Тамир тасарч, хүч барагдав.": "Stamina gone, strength spent.",
  "Тарсан мал хашаандаа орж, хотон бүрдэв.":
    "The scattered livestock came home, and the pen was whole again.",
  "Тонгойж чулуу цохиж гал асааж байна…":
    "Bent low, striking stone for a spark…",
  "Тулаан дуусах хүртэл сүнсний оронгоос гарч чадахгүй.":
    "You cannot leave the spirit realm until the fight ends.",
  "Тулааны хөдөлгөөн дууссаны дараа зэвсгээ солино.":
    "Change weapons once the current move ends.",
  "Тэвш дүүрэн.": "The trough is full.",
  "Тэвш хоосон — мал өлсөж байна!":
    "The trough is empty — the livestock are hungry!",
  Тэмээ: "Camel",
  Тэмүүлэн: "Temüülen",
  "Тэсвэртэй тамир": "Enduring stamina",
  "ТҮВШИН АХИХ": "LEVEL UP",
  Түвшин: "Level",
  "Түлээ нэмлээ.": "Firewood added.",
  "Түлээ цогшиж, гал аслаа.": "The firewood catches and the fire is lit.",
  "ТӨГС ХААЛТ!": "PERFECT PARRY!",
  "ТӨГСГӨЛ!": "THE END!",
  "Төмөр Шулмас унав. Аав ээжийг буцаан өглөө. E — гэртээ буцаж, хамт амьдар.":
    "Tömör Shulmas has fallen and given your parents back. E — go home and live together.",
  "Төмөр Шулмасын төрөлх зан сэргэж, наян есөн хар шидээр давшин дайрахаар зэхэв.":
    "The true nature of Tömör Shulmas woke, and it made ready to strike with eighty-nine black spells.",
  "Төмөр биеийг зөвхөн Хөх тэнгэрийн сэлэм шархдуулна.":
    "Only the Sky-Blue Sword can wound that iron body.",
  "Төмөр хаалга тулаан дуусах хүртэл түгжээтэй.":
    "The iron gate stays locked until the fight ends.",
  "Төмөр шулмас дарагдав": "Tömör Shulmas defeated",
  "Төмөр шулмастай тулаан · дуустал гарахгүй":
    "Fighting Tömör Shulmas · no way out until it ends",
  "Төмөр шулмасыг дар": "Defeat Tömör Shulmas",
  "Төмөр шулмасын жинхэнэ хүч сэргэв": "Tömör Shulmas's true power wakes",
  "Төмөр шулмасын ордноос буцлаа.": "You left Tömör Shulmas's palace.",
  "Төмөр шулмасын тулаан эхэллээ!": "The fight with Tömör Shulmas begins!",
  "Төмөр шулмасын тэнхэл барагдав  J — Сөрөг цохилт":
    "Tömör Shulmas is spent  J — Counter blow",
  "Төмөр шулмасын тэнцвэр алдагдлаа. J — сөрөг цохилт.":
    "Tömör Shulmas is off balance. J — counter blow.",
  "Улаан туяа цахих мөчид": "When the red glare flashes,",
  "Улаан туяа цахих мөчид L дарж, дайралтыг нь няцаа.":
    "When the red glare flashes, press L to turn the blow aside.",
  "Улаан туяа цахих мөчид L / Parry дарж, дайралтыг нь няцаа.":
    "When the red glare flashes, press L / Parry to turn the blow aside.",
  "Унаж явахад хурд +50%": "Speed +50% while riding",
  Уналаа: "Mounted",
  "Унасан баатрын дэргэд": "Take the sword left beside",
  "Унасан баатрын дэргэд үлдсэн сэлмийг ав.":
    "Take the sword left beside the fallen warrior.",
  "Унах морь": "Riding horse",
  "Унах морь авлаа! Гэрийн баруун талд уяа бослоо. E — бууж уях.":
    "Got a riding horse! A hitching line stands west of the ger. E — dismount, hitch.",
  "Унах морь авлаа! Гэрийн баруун талд уяа бослоо. H — бууж уях.":
    "Got a riding horse! A hitching line stands west of the ger. H — dismount, hitch.",
  "Унах морь алга — авдраас ав.": "No riding horse — get one from the chest.",
  "Унтаж байна…": "Sleeping…",
  "Унтаж байна… Zzz": "Sleeping… Zzz",
  Урлал: "Craft",
  Уурга: "Lasso",
  "Уурга хэрэгтэй — авдраас худалдаж ав.":
    "You need a lasso — buy one from the chest.",
  "ХАЗУУЛАВ!": "BITTEN!",
  "ХАР ТӨМӨР ХААЛГА": "THE BLACK IRON GATE",
  "ХАРААЛТ ХААЛГА": "THE CURSED GATE",
  ХУДАЛДАА: "TRADE",
  "Хаалга босголоо — түлхэж нээнэ.": "Gate raised — push it open.",
  "Хаалга нурлаа.": "The gate is down.",
  "Хаалга тулаан дуустал түгжээтэй": "Gate locked until the fight ends",
  "Хаах (P)": "Close (P)",
  "Хавсарга шуурга хуйларч, харанхуй борооны гүнээс хахир хүйтэн инээд хадах мэт сонсогдоно.":
    "A double storm whirled up, and out of the dark rain came laughter, bitter and cold as flint.",
  "Хадгалсан бараа · зарах бол өвгөнтэй арилжаа":
    "Stored goods · trade with the elder to sell",
  "Хамгийн дээд шатны хашаа байна.": "This fence is already the highest tier.",
  "Хар мөрийн хөдөлгөөнийг": "Go and tell the elder",
  "Хар мөрийн хөдөлгөөнийг өвгөнд очиж өгүүл.":
    "Go and tell the elder how the dark trail moves.",
  "Хар төмөр хаалга нээгдсэн": "The black iron gate is open",
  "Хар төмөр хаалгыг Хөх тэнгэрийн сэлэм л нээнэ.":
    "Only the Sky-Blue Sword opens the black iron gate.",
  "Хар төмөр хаалгыг нээ": "Open the black iron gate",
  "Хар төмөр хаалгыг нээхийн тулд Шулмасын баатрыг ялж, Хөх тэнгэрийн сэлмийг ол.":
    "To open the black iron gate, beat the Shulmas Warrior and find the Sky-Blue Sword.",
  "Хар хүлээс тасарч, голомтын бараа дахин бүтэн болов.":
    "The black bonds broke, and the circle of the hearth was whole once more.",
  "Хар үнс салхины өөдөөс мөлхөх мэт хөдөлж, чулуун завсраас хахир инээд цуурайтав.":
    "Black ash crept against the wind, and a bitter laugh echoed from the rocks.",
  "Хараалд автсан таван": "Defeat the five",
  "Хараалд автсан таван сахиулыг дар.": "Defeat the five cursed guardians.",
  "Хараалт талбайг сахих": "Defeat the warrior guarding",
  "Хараалт талбайг сахих баатрыг ял.":
    "Defeat the warrior guarding the cursed grounds.",
  "Хараалт хаалга нээгдсэн": "The cursed gate is open",
  "Хараалт хаалгад хүр": "Reach the cursed gate",
  "Хараалт хаалгыг зорь.": "Make for the cursed gate.",
  "Харвах — сум хэрэгтэй (урлалаар хийнэ)":
    "Shoot — needs arrows (craft them)",
  "Харсан зүгт барина · B дахин · P цуцлах":
    "Builds where you face · B again · P cancel",
  "Хашаа нурлаа.": "The fence is down.",
  "Хашаа нурлаа — мал завсраар бэлчээрт гарч байна!":
    "Fence down — livestock are escaping to graze through the gap!",
  "Хашаа нурлаа — үхэр завсраар бэлчээрт гарч байна!":
    "Fence down — cattle are escaping to graze through the gap!",
  "Мал гарлаа!": "Livestock out!",
  "Үхэр гарлаа!": "Cattle out!",
  "Хашаа хамгааллаа": "The fence held",
  "Хашаа хамгааллаа!": "The fence held!",
  "Хашаа хулгайчийг зогсоов!": "The fence stopped the thief!",
  "Хашаа эвдэрлээ": "The fence broke",
  "Хашаанд баавгай унав!": "A bear fell at the pen!",
  "Хашаанд чоно унав!": "A wolf fell at the pen!",
  "Хоньны ноос зөвхөн зуны улиралд гардаг.": "Sheep give wool only in summer.",
  "Хоол алга. Жимс · загас · ааруул цуглуул.":
    "No food. Gather berries · fish · aaruul.",
  "Хулгайч зугтав.": "The thief fled.",
  "Хулгайч ойртлоо — хашаагаа шалга!":
    "A thief is closing in — check your fence!",
  "ХУЛГАЙЧ ИРЛЭЭ!": "THIEF!",
  "ХУЛГАЙЧ ОЙРТЛОО!": "THIEF NEARBY!",
  "Хулгайчид зодуулж ялагдлаа…": "The thieves beat you down…",
  "Хурга даарч үхэв": "A lamb froze to death",
  "Хурдан хөл": "Swift feet",
  "Хурдан цохилт": "Quick strikes",
  "Хэрэгсэл авах, олзоо зарах": "Buy tools, sell your goods",
  "Хүйтэнд 2 дахин тэсвэртэй": "Twice as hardy in the cold",
  "Хүйтэнд нэрвэгдлээ…": "The cold took you…",
  "Хүлээс тасарлаа.": "The bonds are broken.",
  "Хүлээс тасарлаа. E дарж гэрийн зүг буц.":
    "The bonds are broken. Press E to head home.",
  "Хүлээс тасарч, сүнсний манан сарнив.":
    "The bonds broke, and the spirit mist scattered.",
  "Хүчтэй цохилт": "Heavy blows",
  "ХӨХ ТЭНГЭРИЙН СЭЛЭМ": "THE SKY-BLUE SWORD",
  "Хөх тэнгэрийн сэлмийг ав": "Take the Sky-Blue Sword",
  "Хөх тэнгэрийн сэлмийг авлаа. Хар төмөр хаалга нээгдэв. 1: нударга · 2: сэлэм":
    "You have the Sky-Blue Sword. The black iron gate is open. 1: fists · 2: sword",
  "Хөх тэнгэрийн сэлмийг хараахан олоогүй байна.":
    "You have not found the Sky-Blue Sword yet.",
  "Хөх тэнгэрийн сэлмээ сугаллаа.": "You draw the Sky-Blue Sword.",
  "Хөх тэнгэрийн сэлмээр": "With the Sky-Blue Sword,",
  "Хөх тэнгэрийн сэлмээр хар төмөр хаалгыг нээ.":
    "Open the black iron gate with the Sky-Blue Sword.",
  "Хөх тэнгэрийн сэлэм чиний мэдэлд орсон.":
    "The Sky-Blue Sword is yours now.",
  "Цагаан дарь эх": "White Tara",
  "Цадсан бие": "Well-fed body",
  "Цахилгаан хашаа": "Electric fence",
  "Цахилгаан хашаа хулгайчийг няцаалаа!":
    "The electric fence threw the thief back!",
  Цолмон: "Tsolmon",
  "Цохиж зодохоос урьтаж, анзаарч харж сур.":
    "Learn to watch before you strike.",
  "Цохилт хоорондын зай −25%": "−25% time between blows",
  "Цохилтын хүч +30%": "Strike power +30%",
  Чоно: "Wolf",
  "Чоно ойртлоо — хамгаал!": "A wolf is near — defend!",
  "ЧОНО ОЙРТЛОО!": "WOLF APPROACHING!",
  "Чоно хотонд ойртжээ.": "A wolf nears the pen.",
  "Чоно хотонд ойртжээ. Сүргээсээ холдуулж, амьд үлд.":
    "A wolf nears the pen. Draw it from the herd and survive.",
  "Чононд ялагдлаа…": "The wolf beat you…",
  "Чонын сүнс одлоо.": "The wolf's spirit departs.",
  "Чонын сүнс одлоо — ойрхон аюул үлдлээ.":
    "The wolf's spirit departs — danger nearby remains.",
  "Чоно унав! Голомтын дэргэд өвгөн дээр оч.":
    "The wolf is down! Go to the elder by the hearth.",
  "Чонын хөдөлгөөнийг ажигла": "Watch the wolf's moves",
  "ШУЛМАСЫН БААТАР": "THE SHULMAS WARRIOR",
  "ШУЛМАСЫН СҮНСНИЙ ОРОН": "THE DEMON'S SPIRIT REALM",
  "Шаардлагатай XP": "XP needed",
  "Шар туяа тодрох үед J дарж,": "When the yellow glow rises, press J",
  "Шар туяа тодрох үед J дарж, араатны сул мөчийг ашигла.":
    "When the yellow glow rises, press J and take the beast's opening.",
  "Шар туяа тодрох үед": "When the yellow glow rises,",
  "J / Attack дарж цохь.": "press J / Attack to strike.",
  "Шар туяа тодрох үед J / Attack дарж, араатны сул мөчийг ашигла.":
    "When the yellow glow rises, press J / Attack and take the beast's opening.",
  "Шидэт харваач": "Enchanted Archer",
  "Шинэ бууц! Өвөл — тэвш бэлд.": "New camp! Winter — ready the trough.",
  "Шулмасын баатар": "Shulmas Warrior",
  "Шулмасын баатар зам хаалаа. Хараалтай талбайгаас зугтах аргагүй!":
    "The Shulmas Warrior bars the way. No escape from the cursed ground!",
  "Шулмасын баатар унав. Хөх тэнгэрийн сэлэм газарт үлдлээ.":
    "The Shulmas Warrior has fallen. The Sky-Blue Sword lies on the ground.",
  "Шулмасын баатар унасан. Сэлэм газарт хүлээж байна.":
    "The Shulmas Warrior is down. The sword waits on the ground.",
  "Шулмасын баатартай тулаан үргэлжилж байна.":
    "The fight with the Shulmas Warrior goes on.",
  "Шулмасын баатрыг дар": "Defeat the Shulmas Warrior",
  "Шулмасын зарц": "Shulmas Servant",
  "Шулмасын сүнсний зам: туслахуудыг давж, хараалт хаалгад хүр.":
    "The demon's spirit path: get past the helpers and reach the cursed gate.",
  "Шулмасын сүнсний орноос буцлаа.": "Back from the demon's spirit realm.",
  "Шулмасын сүнсний орон… J — ойрын цохилт, K — сүнсний сум. Туслахууд голын цаана.":
    "The demon's spirit realm… J — close blow, K — spirit arrow. The helpers are across the river.",
  "Шулмасын туслахууд · E — буцах": "The demon's helpers · E — back",
  "Шулмасын туслахууд унав. E — бодит ертөнц рүү буцах · хаалга руу оч.":
    "The demon's helpers have fallen. E — return to the real world · go to the gate.",
  "Шулмасын хүү": "Shulmas Son",
  "Шуурганд тарсан малаа олж,": "Find the livestock the storm scattered",
  "Шуурганд тарсан малаа олж, хотондоо буцаа.":
    "Find the livestock the storm scattered and drive them back to the pen.",
  "Шуурганы мөр тал өөд татжээ.\n\nМалын мөрийг дагаж, харанхуй болохоос урьтаж сүргээ буцаа.":
    "The storm's trail runs out across the steppe.\n\nFollow the tracks and bring the herd back before dark.",
  "Шуурганы мөрийг шинж": "Examine the storm's trail",
  "ШӨНИЙГ ДАВЛАА": "THE NIGHT IS SURVIVED",
  "Шөнийн чоно мал руу дайрлаа!": "A night wolf struck the livestock!",
  "ШӨНИЙН ЧОНО ИРЛЭЭ!": "NIGHT WOLF!",
  "Шөнө боллоо! Мал гадаа. Чоно, баавгай дайрах аюултай":
    "Night! Livestock outside. Wolves and bears may strike",
  "Шөнө боллоо! Мал хашаандаа. Галд дулаац":
    "Night! Livestock penned. Warm yourself at the fire",
  "Шөнө болов — мал бэлчээрт үлдэв! Аюултай!":
    "Night has come — livestock left on pasture! Dangerous!",
  "Шөнө боллоо — үлдсэн малыг хашаанд оруул!":
    "Night! Get the remaining livestock into the pen!",
  "Шөнө дөлөөр хар үүл хуралдан, хар хэрээ гуагалж, хачин муу ёр тал нутгийг нөмрөв.":
    "Deep in the night black clouds gathered, a black raven croaked, and a strange ill omen settled over the steppe.",
  "Эгэлгүй энэ төмөр хуягийг илд жадаар сүлбэшгүй.":
    "No sword or spear will pierce this uncanny iron mail.",
  "Энд хашаа барихгүй.": "No fence here.",
  "Энэ төлөвт алгасах story үе алга.": "Nothing to skip in this state.",
  "Энэ төмөр биеийг энгийн зэвсгээр сүлбэшгүй":
    "Common weapons cannot pierce this iron body",
  "Эрэг дээр зогсож уургална — ус руу бүү ор.":
    "Cast from the bank — do not step into the water.",
  "Эсгий зарах": "Sell felt",
  "Эхлээд E-ээр морь уна, дараа нь G дарж гэр ачна.":
    "Mount with E first, then press G to load the ger.",
  "Эхлээд H-ээр морь уна, дараа нь G дарж гэр ачна.":
    "Mount with H first, then press G to load the ger.",
  "Эхлээд малыг хашаанд оруул (хаалганаас E), дараа нь G.":
    "Pen the livestock first (E at the gate), then G.",
  "Эхний зам цэвэрлэгдлээ. Хараалт хаалганы цаана mini-boss хүлээж байна.":
    "The first path is clear. A mini-boss waits beyond the cursed gate.",
  Ээж: "Mother",
  "Ээж гэртээ": "Mother is home",
  "Ээж саав": "Mother milked",
  "Явах (P)": "Leave (P)",
  "Задарсан гэрийн дэргэд": "By the dismantled ger",
  "E / Space / J — үргэлжлүүлэх": "E / Space / J — continue",
  Ярилцах: "Talk",
  "араатны дайрах мөчийг тань.": "learn when the beast strikes.",
  "араатны сул мөчийг ашигла.": "and take the beast's opening.",
  "баатрыг ял.": "the cursed grounds.",
  зоос: "coins",
  морь: "horse",
  "сахиулыг дар.": "cursed guardians.",
  тэмээ: "camel",
  "төмөр шулмасыг ял.": "bound your mother and father.",
  "хар төмөр хаалгыг нээ.": "open the black iron gate.",
  "хар шуурганы үлдээсэн мөрийг ол.": "and find the black storm's trail.",
  хонь: "sheep",
  "хотондоо буцаа.": "and drive them back to the pen.",
  ямаа: "goat",
  "ҮЛ ТАНИХ ӨВГӨН": "A STRANGE OLD MAN",
  Үргэлжлүүлэх: "Continue",
  Үхэр: "Cow",
  "Үхэшгүй · мод хязгааргүй · зоос хязгааргүй (/).":
    "Deathless · endless wood · endless coins (/).",
  "Үүр цайлаа! Гал түлээд E-ээр малаа бэлчээрт гарга.":
    "Dawn! Light the fire and press E to send the livestock out.",
  "Үүр цайлаа! Галаа түлээд малаа бэлчээрт гарга":
    "Dawn! Light your fire and send the livestock out",
  "Үүр цайлаа! Галаа түлээд малаа бэлчээрт гарга.":
    "Dawn! Light your fire and send the livestock out.",
  "Үүр цайлаа! Дулаацаад ажилдаа ор": "Dawn! Warm up and get to work",
  "Үүр цайхыг хүлээ": "Wait for dawn",
  "Үүрийн цолмон цайхад голомтын гал бөхөж, гадаах малаас ганц ч дуу үл дуулдана.":
    "As the morning star paled, the hearth fire died, and not one sound came from the livestock outside.",
  "Үүрээр өвгөнийг зорь": "Seek the elder at dawn",
  "үл таних өвгөн дээр оч.": "sitting by the hearth.",
  "үлдсэн сэлмийг ав.": "the fallen warrior.",
  үхэр: "cow",
  "Өв соёл": "Heritage",
  "Өв соёлын асуултад хариулах": "Answer questions on heritage",
  "Өвгөн: «Буруу байна. Сайн бэлд, дараа ир.»":
    "Elder: «That's wrong. Prepare well and come again.»",
  "Өвгөн: «Үүрийн гэгээ ортол голомтоо түшиж амар, хүү минь.»":
    "Elder: «Rest by your hearth until first light, my boy.»",
  "Өвгөн: «Өв соёлоо мэддэг үү, хүү минь?»":
    "Elder: «Do you know your heritage, my boy?»",
  "Өвгөн: Тийм ээ, хүү минь. Одоо шар туяаг бүү алд.":
    "Elder: That's it, my boy. Now do not miss the yellow glow.",
  "Өвгөн: Улаан туяа цахих агшинд сөрөөрэй, хүү минь.":
    "Elder: Parry the instant the red glare flashes, my boy.",
  "Өвгөнд мэдээ хүргэ": "Bring word to the elder",
  "Өвгөний бууцнаас зүүн хойших чулуун завсарт очиж, хар шуурганы үлдээсэн мөрийг ол.":
    "Go to the rocks northeast of the elder's camp and find the trail the black storm left.",
  "Өвгөний үгийг сонсож,": "Heed the elder's words,",
  "Өвгөний үгийг сонсож, араатны дайрах мөчийг тань.":
    "Heed the elder's words and learn when the beast strikes.",
  "Өвгөнтэй ярилц": "Speak with the elder",
  "Өвгөнөөс зам мөр, учир явдлыг асуух":
    "Ask the elder about the trail and what befell",
  "Өвс алга — бэлчээрээс E-ээр хад.":
    "No hay — cut some from the pasture with E.",
  "Өвс дууссан — мал өлсөж байна! G-ээр нүү эсвэл өвс өг.":
    "Hay is gone — the livestock starve! Move with G or feed them.",
  "Өвс хадгал! Өвөл ойртож байна.": "Store hay! Winter is drawing near.",
  "Өвөл бэлчээр хөлдсөн — өвс хадахгүй.":
    "Winter has frozen the pasture — no hay to cut.",
  "Өвөл ирлээ! Бэлчээр хөлдөв. Тэвш + гал бэлд.":
    "Winter is here! The pasture froze. Ready trough + fire.",
  "Өвөл ирлээ! Өвсгүй — мал өлсөх, чи даарах аюултай!":
    "Winter is here! No hay — the livestock starve and you freeze!",
  "Өдөр боллоо! Дулаацаж, тэвшээ бэлд": "Day! Warm up and ready the trough",
  "Өдөр боллоо! Ноолуур түү, хурга/ишиг дулаан байлга":
    "Day! Comb cashmere, keep lambs and kids warm",
  "Өдөр боллоо! Түлээ нөөцөл, хашаагаа хүчитгэ":
    "Day! Stock firewood, strengthen your fence",
  "Өдөр боллоо! Өвс, жимс, хоньны ноос түү":
    "Day! Gather hay, berries and sheep wool",
  "Өдөр болов — малаа гаргаагүй байна!":
    "Day has come — your livestock are still penned!",
  "Өдөр болов — малаа гаргаагүй! Өдөржин хашаанд байлгавал өлсөж үхнэ.":
    "Day has come — livestock still penned! Keep them in all day and they'll starve.",
  "Өдөр! Мал хашаандаа өлсөнө — E-ээр бэлчээрт гарга":
    "Day! Livestock starve in the pen — press E to let them graze",
  "Өдөр хашаанд байлгавал өлсөнө — орой л оруулдаг нь дээр.":
    "They starve if penned by day — better bring them in at evening.",
  "Өдөржин хашаанд байлгавал өлсөж үхнэ! E — бэлчээрт гарга.":
    "Kept in the pen all day they'll starve! E — let them out to graze.",
  "Мал хашаандаа өлсөж байна — өглөө бүр бэлчээрт гарга!":
    "Livestock starving in the pen — let them out every morning!",
  "Өлсгөлөн 40%-иар удаан унана": "Hunger falls 40% slower",
  "Өлсөж үхлээ…": "You starved…",
  "ӨЛСӨЖ БАЙНА! ХООЛ ИД!": "STARVING! EAT FOOD!",
  "ӨЛСӨЖ ҮХЭХ ГЭЖ БАЙНА!": "STARVING TO DEATH!",
  "ДААРЧ БАЙНА! ГАЛ ТҮЛ!": "FREEZING! LIGHT A FIRE!",
  "ХҮЙТЭНД НЭРВЭГДЭЖ БАЙНА!": "DYING OF COLD!",
  "F — гал асаа · гэртээ дулаац": "F — light a fire · warm up in the ger",
  "МАЛ ӨЛСӨЖ БАЙНА!": "LIVESTOCK STARVING!",
  "МАЛ ӨЛСӨЖ ҮХЭЖ БАЙНА!": "LIVESTOCK DYING OF HUNGER!",
  "Q дарж ид · эсвэл малдаа өвс өг": "Press Q to eat · or feed hay",
  "Өргөстэй тор": "Barbed wire",
  "өвгөнд очиж өгүүл.": "how the dark trail moves.",
  "өвгөний бууцанд оч.": "camp in the eastern hills.",
  "← Үндсэн цэс": "← Main menu",
  "−1 мал (шөнийн дайралт)": "−1 livestock (night raid)",
  "−1 мал (хашаанд өлсөв)": "−1 livestock (starved in pen)",
  "−1 мал (өвсгүй)": "−1 livestock (no hay)",
  "−1 мал (өлсгөлөн)": "−1 livestock (hunger)",
  "−1 сүнс · амиллаа": "−1 spirit · revived",
  "E — Рашаан авах · дараа нь буцах": "E — Take sacred water · then return",
  "E — Рашаан авах (3 балга · R — уух)": "E — Take sacred water (3 sips · R to drink)",
  "R — Рашаан уух (бүтэн амь)": "R — Drink sacred water (full health)",
  "E — Рашаан авах": "E — Take sacred water",
  "Рашаан дууссан.": "The sacred water is gone.",
  "Амь бүрэн — рашаан хэрэггүй.": "Health is full — no need for sacred water.",
  "Рашаан · дууссан": "Sacred water · empty",
  "Сүүлийн рашаанаа уув — амь дүүрэн. Лонх хоосорлоо.": "Last sip of sacred water — health full. Bottle empty.",
};
