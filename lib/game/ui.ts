...
export function drawHud(ctx: CanvasRenderingContext2D, state: GameState): void {
  ...
  if (
    ...
    if (!state.message.includes("\n")) {
      ...
      const lineHeight = 17;
      const boxHeight = 10 + lines.length * lineHeight;
      const mx = (VIEW_W - textWidth) / 2 - 12;
      const my = VIEW_H - 108 - (boxHeight - 26);
      ctx.fillStyle = "rgba(12,10,8,0.82)";
      ctx.fillRect(mx, my, textWidth + 24, boxHeight);
      ctx.strokeStyle = "rgba(232,197,106,0.45)";
      ctx.strokeRect(mx + 0.5, my + 0.5, textWidth + 23, boxHeight - 1);
      ctx.fillStyle = COLORS.hudText;
      lines.forEach((line, index) => {
        ctx.fillText(line, mx + 12, my + 17 + index * lineHeight);
      });
    }
    ctx.globalAlpha = 1;
  }
