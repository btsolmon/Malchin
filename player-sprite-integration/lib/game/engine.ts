...
import { updateCombat, updateDog, updateProjectiles } from "./combat";
  ...
  const setKey = (code: string, pressed: boolean): void => {
    ...
    switch (code) {
      ...
      case "Space":
        ...
        break;
      case "KeyJ":
        if (pressed) input.attack = true;
        break;
      case "ShiftLeft":
      case "ShiftRight":
        if (pressed) input.dodge = true;
        break;
      case "KeyL":
        if (pressed) input.parry = true;
        break;
      case "KeyK":
        if (pressed) input.shoot = true;
        break;
      case "Enter":
        if (pressed) input.confirm = true;
        break;
...
export function update(state: GameState, dt: number): void {
  ...
  if (state.phase === "playing" && !hitStopped) {
    ...
    // Priority: dodge → parry → ranged/melee.
    if (state.input.dodge || state.player.dodgePhase !== "idle") {
